...
```{r}
 ...
 highly.incomplete.columns=sum(incompelete.dt$percentage >97)
...
# Conclusion
In this report a random forest algorithm is applied to the Human Activity Recognition problem of identifying how well an user performs an exercise. It was possible to construct a procedure similar to the one performed by `r Citet(bib, 1)`. Due to time constraints and incomplete data the results obtained in this report are inferior to the ones in the literature (83% agaisnt 98% accuracy). However, if observed alone,  it was a sucessfull aplication of a machine learning algorithm for classification. It was possible to correctly predict the out of sample error of the accuraccy within the estimated confidence interval `r acct$Accuracy` +- `r acct$AccuracySD` E (`r acc$AccuracyLower`,`r acc$AccuracyUpper`). The feature selection was not implemented in this report, instead we used the result presented in the article, a whole new array of ressults could come from different features, due to time limitations the analisys was restricted to the same set of features to enable a possible comparison. There was insufficient time to perform the  leave-one-out cross-validation.
